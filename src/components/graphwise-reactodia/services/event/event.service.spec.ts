...
describe('EventService', () => {
  ...
  test('should return a no-op subscription when the window is unavailable', () => {
    // Given the window bus is unavailable (e.g. SSR)
    windowServiceMock.getWindow.mockReturnValue(undefined);
    const onClear = jest.fn();

    // When subscribing and dispatching the clear event
    const unsubscribe = subscribe(onClear);
    dispatchClear();

    // Then nothing was registered and tearing down the no-op subscription is safe
    expect(onClear).not.toHaveBeenCalled();
