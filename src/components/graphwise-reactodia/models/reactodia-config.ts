...
import {ReactodiaSeedLink} from './reactodia-seed-link';
...
export interface ReactodiaConfig {
  ...
  /**
   * Pre-resolved edges to place on the canvas on startup. When provided, each link is added to the canvas and we do not
   * request data from the connected sparql endpoint, since this is meant for non-persisted data, like construct and describe queries.
   */
  seedGraph?: ReactodiaSeedLink[];
}
